...
- 🟠 **Quand une section affiche ENFIN une donnée jusqu'ici cachée (sites d'un lieu, photos d'un référentiel), sa qualité devient visible d'un coup : regarder les vignettes rendues AVANT de livrer, et filtrer à l'affichage ce qui ne montre pas l'objet (photo sans licence, reprise d'une publication commerciale)** *(19/09/2026)*
  *19/09/2026 Diako : la nouvelle section « À voir » de /lieu, qui sortait 2 447 sites de l'ombre, montrait une chambre d'hôtel de Tamatave pour Nosy Volana, un selfie de groupe pour Mer d'Émeraude et la voiture d'un loueur pour le monument du 29 mars 1947 à Andasibe — 84 couvertures de sites sont des photos Facebook « reprise avec attribution ». Vu seulement en regardant la capture, types et tests étaient verts*
- 🟠 **Un repli qui RÉUSSIT en silence cache la panne du chemin principal : quand un mode de secours prend le relais (base64 après multipart, second fournisseur, cache), journaliser l'échec du premier mode, pas seulement l'échec final — sinon la panne dure tant que le repli tient** *(19/09/2026)*
  *19/09/2026 Diako : depuis début septembre, o2switch refusait en 406 tout envoi multipart sur le domaine. Le site s'en tirait par son repli JSON base64 sans rien écrire nulle part : chaque photo partait deux fois (le double du temps d'envoi en 3G) et le blocage est resté invisible deux semaines, découvert seulement en voulant poser deux couvertures depuis une session*
- 🟠 **Un chiffre porte par une fiche de la base Diako AVEC sa source ne prouve pas que la source dit ce chiffre : relire la source elle-meme avant de le publier, et ne garder que ce qu'on y lit mot pour mot** *(19/09/2026)*
  *19/09/2026 serie 2 : la fiche evenement 'Campagne du girofle' annoncait ~90 % de la production nationale et 18 000 producteurs en citant un article Mongabay ; l'article relu ne porte ni l'un ni l'autre (il dit Analanjirofo = 'dans la foret des girofliers', recolte octobre-decembre, rituel tsaboraha). Les deux chiffres ont ete retires de la publication*
- 🔴 **Un bot dont l'interlocuteur ecrit depuis son TELEPHONE ne lui demande jamais un element technique (chemin de fichier, slug, identifiant, auteur ou licence de photo) : il cherche lui-meme, et si une piste manque il agit avec ce qu'il a. La consigne l'interdit ET le code le rattrape : quand un verbe d'action a impose un outil et que le modele repond par une question, on lance la competence avec la demande comme sujet** *(20/09/2026)*
  *20/09/2026 premier vrai echange avec Andry sur le bot Diako : « fais une publication photo belle de Nosy Be pour souhaiter une bonne dimanche » a recu « Je n'ai pas de chemin d'image a utiliser. Quel(s) fichier(s) photo(s) veux-tu joindre ? ». Sa reponse : « regle ca, il faut que le bots soit tout capable ». Trois essais de suite reclamaient chemin, auteur et licence*
- 🟠 **Une image servie par NOTRE site est une COPIE REDIMENSIONNEE, jamais l'original : pour publier sur une page, remonter au fichier source par sa colonne d'origine (cover_source), et MESURER les pixels du fichier telecharge au lieu de juger son poids. Un plancher de definition se pose sur TOUS les chemins, pas seulement sur celui qu'on vient d'ecrire** *(20/09/2026)*
  *20/09/2026 page Di'ako : Andry a signale que les photos des publications n'etaient pas HD. La couverture du parc Masoala servie par diako.fonenako.mg fait 648x1152 pour 158 Ko, son original Wikimedia 4224x3168 pour 5757 Ko ; Ankarana 1200x1600 contre 8256x5504. Cause : le plancher LARGEUR_MINI n'etait applique qu'aux photos de Commons, _depuis_base ne verifiait aucune dimension et passait en premier, et photo_pour prenait la premiere qui se telechargeait*
