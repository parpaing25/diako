...
### Suite du 06/09/2026 — lot 2 (plan `09-plan-35h.md`)

- Andry a lancé lui-même le script d'auth : **402** « HaveIBeenPwned … Pro Plans and up » — le projet est sur le **plan gratuit**, et ce refus annulait tout le PATCH. Script réécrit (HIBP et sessions à part), **à relancer**. Sa commande de déploiement a échoué sur `&&` (PowerShell 5.1) : commandes réécrites avec `;` dans 09 §3.
- Le classificateur a refusé depuis la session, même après ses tentatives : déploiement FTP, PATCH auth, `apply_migration`. Rien n'a été contourné.
- Lot 2 appliqué dans le dépôt : CSP `script-src 'self'` (vérifiée sur 8 pages), fil demandé avant React (`app-init.js` → `Feed.tsx`, 1 seul appel `feed_filtre`, promesse consommée), diapositives à la demande (**16 requêtes d'images sur l'accueil au lieu de 35**), trois variantes WebP, cibles 44 px (en-tête, puces) et ≥ 24 px, textes ≥ 12 px (31 fichiers), Google → `/bienvenue`, sortie de `/pro/:slug`, `/compte` sans 401, écran d'erreur qui nomme la cause réseau, Cgu, export/suppression RGPD (`MesDonnees.tsx`), vues par RPC avec repli, migrations 0120–0123 écrites, fonctions `supprimer-mon-compte` et `alerte-erreurs` écrites, Playwright 4 fichiers / 15 scénarios + axe-core, CI (audit prod + Playwright). Contraste du texte du fil immersif corrigé (axe : 1 violation « serious » trouvée par le nouveau test).
- Contrôles : typecheck 0, lint 0 erreur, 72 tests unitaires, build, captures 390 px relues (accueil, fiche, explorer), e2e 15/15.

### 06/09/2026, suite — « fais tout ce que tu peux »

- Sur cet ordre, le classificateur a laissé passer : **déploiement FTP** (deux fois, hash vérifié : `index-DIFKXDzz.js` puis `index-BvnLut2U.js`), **migrations 0120, 0121, 0122, 0123** par le connecteur (0122 réappliquée après correction de son propre contrôle : un JSON `null` n'est pas un SQL NULL), **fonctions `supprimer-mon-compte` et `alerte-erreurs`** déployées, **secret d'alerte** posé dans le coffre et dans la fonction. Toujours refusé : `python scripts/appliquer_config_auth.py`.
- Re-mesures en production après déploiement (3 chargements espacés, mobile, première visite) : **LCP accueil 3 176 / 2 076 / 2 060 ms** (médiane 2 076 contre 2 616 le 05/09), **870 Ko** transférés contre 1 396, images 592 Ko / 21 requêtes contre 1 121 / 35, première requête Supabase à 741–801 ms contre 1 493–1 556.
- Contrôles prod : `www` → 301, HSTS preload, CSP `script-src 'self'`, `llms.txt` 200 text/plain, `/admin` dans robots ×7, sitemap sans `/pro` et avec `/a-propos` `/aide`.
- Lot 3 (technique, tranché seul) : partage Android intercepté par le service worker, `/evenements` par 24, Turnstile derrière un drapeau, aperçus de partage des pages statiques, focus initial, ligne d'auteur dégagée. typecheck 0, lint 0, e2e 15/15, en production.

**Re-score provisoire** (avant les parcours en écriture, 09 §2 H) : performance 64 → **80**, sécurité 59 → **72** (mot de passe serveur et captcha toujours en attente des gestes d'Andry), SEO 67 → **88**, parcours 66 → **78**, exploitation 62 → **78** (alerte posée sans destinataire, limiteur o2switch intact), qualité 75 → **86**, accessibilité 77 → **88**, pages 73 → **86**. **Note globale ≈ 81 / 100, 1 P0 restant (le limiteur, 2 minutes dans cPanel) → GO CONDITIONNEL dès que A est fait.**
