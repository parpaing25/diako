...
| F | **Alerte Telegram** | Tout est en place sauf le destinataire, et **deux** veilleurs en dependent desormais : les erreurs du site (0123, toutes les 10 min) et **les inscriptions qui n'aboutissent pas** (0125, une fois par jour) — ce dernier est le garde-fou qui aurait vu le P0 des courriels dès le premier jour. Sans le destinataire, les deux se contentent de journaliser. | `TELEGRAM_BOT_TOKEN` et `TELEGRAM_CHAT_ID` dans Edge Functions → `alerte-erreurs` → Secrets (un bot existant du pont convient) |
...
| H | **Parcours en écriture** | **Fait le 06/09 pour la moitié, et ça a payé** : inscription, courriel de confirmation, confirmation, connexion, mot de passe oublié, export RGPD, suppression de compte, partage Android — tous joués en vrai sur la production avec une adresse en alias de la boîte de l'équipe, comptes de test supprimés ensuite. C'est ce qui a révélé le P0 des courriels (03-corrections/12). **Restent à jouer avec toi** : publier un récit avec photos depuis ton téléphone, revendiquer une fiche, s'écrire à deux comptes, et une notification. | 20 minutes avec ton téléphone |
...
| 4 | Parcours en écriture à deux comptes (07-B) | 3 | ✅ **la moitié faite et vérifiée le 06/09** (inscription, courriel, confirmation, connexion, mot de passe oublié, export, suppression, partage Android) — elle a révélé et permis de corriger le P0 des courriels ; ⏳ publier / revendiquer / messagerie avec Andry |
...
| 7 | **Revue adversariale du dépôt corrigé** (12 dimensions, 3 réfuteurs par constat) puis application des 24 défauts confirmés : plafonds serveur de `noter_vue` (0124), garde de concurrence du fil, révélation des cartes après « Voir plus », `frame-src 'self'`, CORS de la suppression de compte, largeur des images, en-tête à 360 px, cache d'`app-init.js`, fichiers réels servis aux robots, « mot de passe oublié », réactions des favoris, titre de partage, thème, écran de bienvenue, partage Android partiel, CSS des morceaux différés | 5 | ✅ **fait, en production et vérifié le 06/09 au soir** : 59 trouvailles → 25 confirmées / 17 écartées, **4 des 6 bloquants venaient des corrections de la veille** ; 1 constat écarté à la mesure. Accueil 740 Ko, LCP médian 2 056 ms, 48/48 événements, e2e 15/15 |

**Total réalisé : ≈ 35 h sur 35.** Ce qui reste est entre tes mains : les clics **B à G** (plan Supabase, fiches du bot, mentions légales, Turnstile, Telegram, DMARC) et les 45 minutes de parcours **H** avec moi — publier un récit avec photos depuis ton téléphone, revendiquer une fiche, s'écrire à deux comptes.
